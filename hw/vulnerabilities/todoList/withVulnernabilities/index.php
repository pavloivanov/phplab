...
$action = isset($_SESSION['logged']) ? 'logged' : 'signin';
...
switch ($action) {
    ...
    case 'registration':
        ...
        break;
    case 'logged':
        $username = isset($_REQUEST['username']) ? $_REQUEST['username'] : $_SESSION['logged']['username'];
        $todoList = readTasks($username);
    ...
    case 'addTask':
        if (isset($_REQUEST['task'])) {
            addTask($_REQUEST['task'], $_SESSION['logged']['username']);
            header('location: index.php');
            exit();
        }
